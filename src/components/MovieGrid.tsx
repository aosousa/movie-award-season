...
const MovieGrid = () => {
  ...
  const changeCategoryData = useCallback(
    () =>
      setCategoryData(data.filter((c: Category) => c.category === category)),
    [category, data]
  );
  ...
  return (
    <div>
      <div className="flex flex-col p-4">
        <h1 className="text-3xl font-bold mx-auto">Movie Award Season</h1>
      </div>

      <div className="pl-2">
        <div className="flex flex-col">
          <div className="mx-auto">
            <label htmlFor="year" className="font-semibold text-xl mr-2">
              Year
            </label>
            <select
              name="year"
              id="year"
              className="w-20 h-8 border-2 border-gray-500 rounded-md"
            ...
            <label
              htmlFor="category"
              className="font-semibold text-xl ml-4 mr-2"
            >
              Category
            </label>
            <select
            ...
            >
              ...
              <option value="Best Original Screenplay">
                Best Original Screenplay
              </option>
              <option value="Best Adapted Screenplay">
                Best Adapted Screenplay
              </option>
              <option value="Best Animated Film">Best Animated Film</option>
              <option value="Best International Film">
      ...
      <div className="grid grid-flow-row grid-rows-max xs:grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-4 px-4 rounded-md mx-2 p-4">
        {categoryData.map((category: Category) =>
          category.winners.map(
            (winner: Winner, idx: number) =>
              winner.winner_name !== "N/A" && (
                <MovieItem key={idx} winner={winner} />
              )
          )
        )}
      </div>
